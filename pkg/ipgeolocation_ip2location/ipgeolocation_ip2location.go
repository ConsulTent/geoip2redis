package ipgeolocation_ip2location
...
func IpGeoLocation_Merge(mmcsv string, lcsv string, tz bool, tmpdir ...string) string {
	var TempFile string
	var i, x int
	//	var long uint32
	...
	fmt.Printf("Loading ipgeolocation Country data into hash table.\n")
	...
	for _, block := range blocks {
		...
		if i != 1 {
			...
			if len(network_cidr) == 0 {
				fmt.Println("Received blank value for CIDR in ipgeolocation source, exiting.")
				os.Exit(1)
			}
