...
//	long = ip2long(cmds.Rawip)
		...
		fmt.Printf("maxmind->ip2location (c) 2020 ConsulTent Ltd. v%s-%s\n", pver, gitver)
 ...
 for _, location := range locations {
				 ...
				 for _, cell := range location {
								 if i != 1 {
												 ...
												 if x == 1 { geonameid, _ = strconv.Atoi(cell) }
												 if x == 5 || x == 6 || x == 8 || x == 11 {
...
// End of locations, now we do the data.
	...
	/*							 if DEBUG == true {
								 ...
								 if x > 10 { fmt.Printf("Warning: there are %d extra columns.",x-10) }
			 								 if i != 1 {
			 									 			 // Keep: 1,2
															 if x == 1 { network_cidr = cell }
			 												 if x == 2 { geonameid, _ = strconv.Atoi(cell) }
															 if x == 3 {
																 if geonameid == 0 { geonameid, _ = strconv.Atoi(cell)
																 		if DEBUG == true { fmt.Println("geonameid was 0") }
																	}
															 }
			 								 } //SkipHeader
									x++
...
//ip2record = strings.Split(fmt.Sprintf("\"%d\",\"%d\",%s",iPv4ToUint32(ipStart), iPv4ToUint32(ipEnd), outline), ",")
							if iPv4ToUint32(ipStart) == 16777216 && i == 2 { _, err := io.WriteString(csvOut,fmt.Sprintf("\"0\",\"16777215\",\"-\",\"-\",\"-\",\"-\"\n"))
								if err != nil {
								panic(err)
								}
								if DEBUG == true { fmt.Println("iPv4ToUint32(ipStart) == 16777216 && i == 2") }
							}
							...
							if len(outline) > 1 {
								...
								if DEBUG == true { fmt.Printf("outlinesize: %d\n",len(strings.Split(outline,","))) }
							} else { if DEBUG == true { fmt.Printf("outlinesize: %d - SKIPPED\n",len(strings.Split(outline,","))) } }
